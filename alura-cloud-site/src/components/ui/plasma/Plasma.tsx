...
void mainImage(out vec4 o, vec2 C) {
  ...
  for (vec2 r = iResolution.xy, Q; ++i < 60.0; O += oTemp.w/d*oTemp.xyz) {
    ...
    Q = p.xz;
    float c = cos(p.y+11.0-T);
    float s = sin(p.y+11.0-T);
    mat2 rot = mat2(c, -s, s, c);
    Q *= rot;
    p.xz = Q;
    
    z += d = (abs(sqrt(length(Q*Q)) - .25*(5.+S.y))/3.+8e-4) * uStepScale;
    oTemp = 1.+sin(S.y+p.z*.5+S.z-length(S-p)+vec4(2,1,0,8));
    if (i >= uQuality) break;
  }
...
export const Plasma = ({
  ...
  useEffect(() => {
    ...
    if (mouseInteractive) {
      containerEl.addEventListener('mousemove', handleMouseMove as any, { passive: true });
    }
    ...
    return () => {
      ...
      if (mouseInteractive && containerEl) {
        containerEl.removeEventListener('mousemove', handleMouseMove as any);
      }
      try {
